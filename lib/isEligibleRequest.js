 ...
 */
const hasBody = (req) => {
  const transferEncodings = asHeaderValues(req.headers['transfer-encoding']);
  const contentLengths = asHeaderValues(req.headers['content-length']);
...
const hasAcceptableContentType = (req) => {
  return asHeaderValues(req.headers['content-type']).some(contentType =>
